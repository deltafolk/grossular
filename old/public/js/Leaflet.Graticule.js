...
// 	},
    ...
    __format_lng: function(lng) {
        ...
        // todo: format type of float
        if (lng > 180) {
            return '' + (360 - lng.toFixed(4)) + 'W';
        }
        else if (lng > 0 && lng < 180) {
            return '' + lng.toFixed(4) + 'E';
        }
        else if (lng < 0 && lng > -180) {
            return '' + (lng.toFixed(4) * -1) + 'W';
        }
        else if (lng == -180) {
            return '' + (lng.toFixed(4) * -1);
        }
        else if (lng < -180) {
            return '' + (360 + lng.toFixed(4)) + 'W';
        }
...
function point_north(point_e, point_n) {
  ex = point_e
  fx = point_n

  c15 = Math.sqrt ( (a_axis * a_axis) - (b_axis * b_axis) ) / a_axis ;
  c16 = Math.sqrt ( (a_axis * a_axis) - (b_axis * b_axis) ) / b_axis ;
  c17 = c16 * c16;
  c18 = + (a_axis * a_axis) / b_axis;
  
  gx = fx * (Math.PI) / 180;
  ix = Math.trunc((fx / 6)+31);
  jx = 6 * ix - 183;
  
  hx = ex * (Math.PI) / 180;
  kx = + gx - ((jx * (Math.PI)) / 180);
  lx = Math.cos(hx) * Math.sin(kx);
  qx = Math.sin(2 * hx);
  rx = + qx * (Math.cos(hx) * Math.cos(hx));
  
  sx = hx + (qx/2);
  tx = ((3 * sx) + rx) / 4;
  ux = (5 * tx + rx * (Math.cos(hx) * Math.cos(hx))) / 3 ; // check
  vx = (3/4) * c17;
  wx = (5/3) * ( vx * vx ); 
  xx = (35/27) * Math.pow(vx, 3);
  
  mx = (1/2) * Math.log(( 1 + lx)/(1-lx));
  nx = Math.atan((Math.tan(hx))/ Math.cos(kx))- hx;
  ox = ( c18 / Math.sqrt(1 + c17 * ( Math.cos(hx) * Math.cos(hx) ) ) ) * 0.9996; //check
  px = (c17 / 2) * mx * mx * Math.pow(Math.cos(hx), 2);
  yx = 0.9996 * c18 * (hx - ( vx * sx ) + ( wx * tx ) - ( xx * ux ) ) ;
  
  eastx = mx * ox * (1 + px / 3) + 500000;
  northx = nx * ox * ( 1 + px ) + yx;
  
  return northx.toFixed(0);
  
}

function point_east(point_e, point_n) {
  ex = point_e
  fx = point_n
